...
class Model(object):
    def __init__(self, config):
        ...
        self.lr = config['lr']
        # self.lr = tf.maximum(config["lr"], 0.001 / tf.log(20000.) * tf.log(
        ...
        used = tf.sign(tf.abs(self.char_inputs))  # 输入char_inputs为shape为[batch_size,sentences_length],
        # tf.abs的作用是将取每个元素的绝对值，tf.sign为sign函数,指大于0的sign值为1
        length = tf.reduce_sum(used, reduction_indices=1)   # 计算batch中每个句子的长度
        ...
        if self.model_type == 'bilstm':
            ...
            model_outputs = tf.nn.dropout(model_outputs, self.dropout)

            # GRU层
            model_outputs = self.GRU_layer(model_outputs, self.gru_dim)
    ...
    def GRU_layer(self, inputs, gru_dim, name=None):
        with tf.variable_scope("gru_layer" if not name else name):
    ...
    def project_layer_bilstm(self, lstm_outputs, name=None):
        ...
        with tf.variable_scope("project" if not name else name):
            with tf.variable_scope("hidden"):
                W = tf.get_variable("W", shape=[self.gru_dim, self.lstm_dim],
                                    dtype=tf.float32, initializer=self.initializer)

                b = tf.get_variable("b", shape=[self.lstm_dim], dtype=tf.float32,
                                    initializer=tf.zeros_initializer())
                output = tf.reshape(lstm_outputs, shape=[-1, self.gru_dim])
